...
MQTTClient::MQTTClient(
  ...
  char* mqttClientName) :
  MQTTClient(NULL, NULL, mqttServerIp, NULL, NULL, mqttClientName, mqttServerPort)
{
}
MQTTClient::MQTTClient(
  char* mqttServerIp,
  ...
  char* mqttPassword,
  char* mqttClientName) :
  MQTTClient(NULL, NULL, mqttServerIp, mqttUsername, mqttPassword, mqttClientName, mqttServerPort)
{
}
...
MQTTClient::MQTTClient(
  ...
  char* mqttClientName,
  short mqttServerPort) :
  MQTTClient(wifiSsid, wifiPassword, mqttServerIp, NULL, NULL, mqttClientName, mqttServerPort)
{
}

// Warning : for old ructor support, this will be deleted soon or later
MQTTClient::MQTTClient(
  char* wifiSsid,
  char* wifiPassword,
  char* mqttServerIp,
  ...
  bool enableWebUpdater,
  bool enableSerialLogs) :
  MQTTClient(wifiSsid, wifiPassword, mqttServerIp, mqttUsername, mqttPassword, mqttClientName, mqttServerPort)
{
  if (enableWebUpdater)
  ...
  mMqttClient(mqttServerIp, mqttServerPort, mWifiClient)
{
  ...
  mMqttClient.setCallback([this](char* topic, byte * payload, unsigned int length) {
  ...
  mShowLegacyructorWarning = false;
}
...
void MQTTClient::enableDebuggingMessages( bool enabled)
{
  mEnableSerialLogs = enabled;
}

void MQTTClient::enableHTTPWebUpdater( char* username,  char* password,  char* address)
{
  if (mHttpServer == NULL)
  {
...
void MQTTClient::enableHTTPWebUpdater( char* address)
{
...
void MQTTClient::enableMQTTPersistence()
{
  mMqttCleanSession = false;
}

void MQTTClient::enableLastWillMessage( char* topic,  char* message,  bool retain)
{
...
void MQTTClient::loop()
{
  ...
  // Old ructor support warning
  if (mEnableSerialLogs && mShowLegacyructorWarning)
  {
    mShowLegacyructorWarning = false;
    Serial.print("SYS! You are using a ructor that will be deleted soon, please update your code with the new ruction format.\n");
  }
...
void MQTTClient::subscribe(const String &topic, MessageReceivedCallback messageReceivedCallback)
{
...
void MQTTClient::setUsername(char* username)
{
  MQTTClient(NULL, NULL, mMqttServerIp, username, mMqttPassword, mMqttClientName, mMqttServerPort);
}

void MQTTClient::setPassword(char* password)
{
  MQTTClient(NULL, NULL, mMqttServerIp, mMqttUsername, password, mMqttClientName, mMqttServerPort);
}

void MQTTClient::setIPAddress(char* serverIP)
{
  MQTTClient(NULL, NULL, serverIP, mMqttUsername, mMqttPassword, mMqttClientName, mMqttServerPort);
}

void MQTTClient::setClientName(char* clientName)
{
  MQTTClient(NULL, NULL, mMqttServerIp, mMqttUsername, mMqttPassword, clientName, mMqttServerPort);
}

void MQTTClient::setPort(short MQTTPort)
{
  MQTTClient(NULL, NULL, mMqttServerIp, mMqttUsername, mMqttPassword, mMqttClientName, MQTTPort);
}

void MQTTClient::executeDelayed( long delay, DelayedExecutionCallback callback)
{
...
void MQTTClient::mqttMessageReceivedCallback(char* topic, byte* payload, unsigned int length)
{
